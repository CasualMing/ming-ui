 ...
 * @LastEditTime: 2021-12-02 09:55:24
 * @Description: 布局组件
-->
<template>
    <div class="layout">
        <!-- 顶部导航 -->
        <div class="top-nav" v-if="showNavMenu">
        ...
        <!-- 内容区域 -->
        <div class="view-wrapper" v-if="isContent" :key="$route.fullPath">
            <router-view />
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        generateTopNav() {
            ...
            if (CommonApi) {
                const menuStateStr = CommonApi.getPlatformParam({
                    type: "sessionStorage",
                    key: "menuState",
                });
                if (!menuStateStr) return false;
                const { menuList, currentMenuId, currentMenuURL } =
                    JSON.parse(menuStateStr);
                ...
                // 获取页面路由
                pageRoutes = this.$route.matched.filter((e) => {
                    if (e.meta && e.meta.text) {
                        if (!menuRoutes.length) {
                            ...
                            if (idx == -1) {
                                ...
                                newMenuURL =
                                    currentMenuURL.substring(0, idx + 1) +
                                    e.path;
                            }
